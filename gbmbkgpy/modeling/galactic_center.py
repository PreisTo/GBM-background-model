...
class GC_fixed:
    ...
    def _calc_gc_rates_one_det(self, det_response):
        ...
        with progress_bar(
            ...
            for i in range(len(det_response.Ebin_in_edge) - 1):
                ...
                flux_vec[i] = (
                    0.5 * (x2 - x1) * (self._spectrum(x1) + self._spectrum(x2))
                )

            for time in range(self._Ntime):
                # transposing is needed due to numpy broadcasting rules
                total_resp_matr = np.sum(
                    (det_response.response_array.T * weights[time, :]).T, axis=0
                )
    ...
    def _calc_gc_weights(self, occ_mask, det_response):
        ...
        # cart_coords_grid = np.zeros((3, self._Ngrid))
        # cart_coords_grid[0] = x_val
        # cart_coords_grid[1] = y_val
        # cart_coords_grid[2] = z_val
        cart_coords_grid = det_response.points  # cart_coords_grid.T
        ...
        for time in range(self._Ntime):
            ...
            b = np.arcsin(np.clip(points_transformed[:, 2], -1, 1))
